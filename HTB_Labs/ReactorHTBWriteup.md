...
- Cracking unsalted MD5 hashes with John the Ripper and the rockyou wordlist
...
32 hex chars = **raw MD5, unsalted**. A fast, unsalted hash like this falls quickly to a dictionary attack, so crack it with **John the Ripper** and the rockyou wordlist:
...
This is exactly why real apps use slow, salted hashes (bcrypt/scrypt/Argon2), which resist wordlist attacks.
...
- Fast, unsalted MD5 falls to a wordlist in seconds with John the Ripper. Slow, salted KDFs (bcrypt/scrypt/Argon2) exist for exactly this.
